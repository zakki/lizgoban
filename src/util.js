...
export const to_i = x => (x | 0)
export const to_f = x => (x - 0)
export const to_s = x => (x + '')
export const xor = (a, b) => (!a === !!b)
export const truep = x => (x || x === 0 || x === '')
export const do_nothing = (...args) => {}
export const identity = x => x
export const clip = (x, lower, upper) =>
...
export const each_value = (h, f) => each_key_value(h, (_, v) => f(v))  // for non-array
...
export const around_idx_diff = [[1, 0], [0, 1], [-1, 0], [0, -1]]
export const around_idx = ([i, j]) => {
...
function do_debug_log(arg, limit_len) {
    ...
    const s = to_s(arg), over = s.length - HALF * 2
...
// for engine (chiefly)
export const common_header_length = (a, b) => {
...
export const set_error_handler = (process, handler) => {
    ['stdin', 'stdout', 'stderr'].forEach(k => process[k].on('error', handler))
    process.on('exit', handler)
}

// avoid letters for keyboard operation in renderer.js
export const normal_tag_letters = 'bdefghijklmnorstuwy'
export const last_loaded_element_tag_letter = '.'
export const start_moves_tag_letter = "'"
export const endstate_diff_tag_letter = "/"
export const tag_letters = normal_tag_letters + last_loaded_element_tag_letter +
