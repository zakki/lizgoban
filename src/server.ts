...
class ClientWindow {
    ...
    initClient() {
        each_key_value(api, (channel, handler) => {
            ...
            this.socket.on(channel,
                simple ? (...a) => handler(...a) : api_handler(channel, handler))
        })
        ...
        this.socket.on('busy', (subchannel, ...args) => busy_handler_for(subchannel)(...args))

        this.socket.on('close_window_or_cut_sequence', () => {
            stop_auto()
            get_windows().forEach(win => (win.socket === this.socket) &&
                close_window_or_cut_sequence(win))
        })
    }
}

let windows: ClientWindow[] = [], last_window_id = -1

/*
function window_prop(win: ClientWindow) {  // fixme: adding private data impolitely
...
function window_for_id(window_id: number) {
    return get_windows().find(win => win.prop.window_id === window_id)
}
...
function new_window(socket: SocketIO.Socket, default_board_type) {
    ...
    //const onf_key = 'window.id' + window_id
    ...
    let board_type = null
    let previous_board_type = null
    const win = new ClientWindow(socket) // get_new_window('index.html', { show: false })
    const prop = win.prop
    merge(prop, {
...
function renderer_gen(channel, win_prop_p, ...args) {
    ...
    get_windows().forEach(win => win.socket
        .emit(channel, ...(win_prop_p ? [win.prop] : []),
            ...args))
}
...
const simple_api = {
    unset_busy,
    //update_menu,
    toggle_board_type, toggle_let_me_think,
    //copy_sgf_to_clipboard,
    set_endstate_diff_from,
}
const api = merge({}, simple_api, {
    restart, new_window, init_from_renderer,
    //toggle_sabaki,
    toggle_pause,
...
function toggle_board_type(window_id, type) {
    ...
    const { board_type, previous_board_type } = win.prop
    const new_type = (type && board_type !== type) ? type : previous_board_type
    set_board_type(new_type, win, !type)
}
function set_board_type(type, win: ClientWindow, keep_let_me_think = false) {
    const prop = win.prop, { board_type, previous_board_type } = prop
...
function toggle_board_type_in_let_me_think() {
    ...
    const current_type = win.prop.board_type
...
function renderer_preferences() {
    return aa2hash(stored_keys_for_renderer.map(key => [key, store.get(key)]))
}
