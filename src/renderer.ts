...
function handle_mouse_on_goban(canvas: HTMLCanvasElement, coord2idx, read_only, tag_clickable_p) {
...
function add_mouse_handlers_with_record(canvas, handlers, hover_updater?) {
    ...
    each_key_value(handlers, (k: string, f) => {
...
function mouse2coord(e: MouseEvent | TouchEvent) {
...
function mouse2idx(e: MouseEvent | TouchEvent, coord2idx): Coordinate {
    const [i, j] = coord2idx(...mouse2coord(e))
    return (0 <= i && i < board_size && 0 <= j && j < board_size) && [i, j]
}
function mouse2move(e: MouseEvent | TouchEvent, coord2idx) {
